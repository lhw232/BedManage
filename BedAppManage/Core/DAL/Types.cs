...
namespace BedAppManage.Core.DAL
{
    ...
    public class Types
    {
        const string CODE_PATH = "DAL.Types.";
        SqlParameter[] parms = null;

        string SQL_INSERT = "INSERT INTO Types(name,img)VALUES(@name,@img)";
        string SQL_DELETE = "DELETE FROM Types WHERE no=@no";
        string SQL_UPDATE = "UPDATE Types SET name=@name,img=@img WHERE no=@no";
        string SQL_GETENTITY = "SELECT * FROM Types WHERE no=@no";
        string SQL_EXISTS_BY_ID = "SELECT * FROM Types WHERE no=@no";
        string SQL_EXISTS_BY_NAME = "SELECT * FROM Types WHERE name=@name";
        string SQL_GETLIST = "SELECT * FROM Types";
        string PRO_GETLIST = "pro_getList_types";
        ...
        public bool Insert(TypesInfo entity)
        {
            try
            {
            ...
            {
                throw new Exception(CODE_PATH + "Types.Insert(...):" + ex.Message);
            }
        }
        #endregion
        ...
        public bool Update(TypesInfo entity)
        {
            try
            {
        ...
        public DataTable GetDataList(string sqlFilter, string orderBy)
        {
            ...
            {
                throw new Exception(CODE_PATH + "Types.GetDataList(...):" + ex.Message);
            }
        }
        ...
        #region ExistsByID 判断系统中是否存在具有相同ID的商品分类表
        /// <summary>
        /// 判断系统中是否存在具有相同ID的商品分类表；
        /// </summary>
        /// <param name="no">分类编号</param>
        ...
        #region ExistsByName 判断系统中是否存在具有相同名称（或标题）的商品分类表
        /// <summary>
        /// 判断系统中是否存在具有相同名称（或标题）的商品分类表；
        /// </summary>
        ...
        public bool ExistsByName(string name)
        {
            ...
            {
                throw new Exception(CODE_PATH + "Types.ExistsByName(...):" + ex.Message);
            }
        }
        #endregion
        ...
        SqlParameter[] GetParametersForAdd(TypesInfo entity)
        {
            return new SqlParameter[]
                {
                    SQLHelper.MakeInParam("@name", SqlDbType.VarChar, 20, entity.name),
                    SQLHelper.MakeInParam("@img", SqlDbType.VarChar, 100, entity.img)
                };
        }
        ...
        /// <param name=" entity">实体</param>
        /// <returns>SqlParameter</returns>
        SqlParameter[] GetParametersForUpdate(TypesInfo entity)
        {
            return new SqlParameter[]
                {
                    SQLHelper.MakeInParam("@name", SqlDbType.VarChar, 20, entity.name),
